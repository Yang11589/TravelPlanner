...
const mimeFromImage = image => image.match(/^data:([^;,]+)/i)?.[1]?.toLowerCase();
...
function ImageZoom({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key !== "Tab") return;
      const focusables = overlayRef.current?.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])');
